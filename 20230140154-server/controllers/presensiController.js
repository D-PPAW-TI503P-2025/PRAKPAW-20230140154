 	...
 	const { Presensi } = require("../models"); // models itu biar server / backendnya itu biar tau tabel tujuannya
  ...
  try {
    ...
    if (!recordToDelete) {
      return res
        .status(404) //ini kalo linknya tidak ada
        .json({ message: "Catatan presensi tidak ditemukan." });
    }
    if (recordToDelete.userId !== userId) {
      return res
        .status(403) //nomor akses ditolak karena tidak sesuai dengan pemilik presensi
